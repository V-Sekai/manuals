...
- **X-axis (Latency):** Age of the task in days (reversed, older tasks on the left).
...
- **Interpretation:** Top-left tasks are high-priority and older—consider these established priorities.
...
- **X-axis (Effort):** Estimated work required (lower is better).
- **Y-axis (Latency):** Age of the task in days (reversed, older tasks at top).
- **Pareto Frontier:** Dotted line shows the optimal trade-off between effort and age.
- **Interpretation:** Top-left tasks are low effort and older—these are good candidates for quick progress on established tasks.
...
fig_el.update_yaxes(showgrid=True, gridcolor='gray', gridwidth=1, autorange="reversed")
fig_el.show()

print("""
## Ranked Task List

The tasks ranked by starvote priority (higher is better):
""")

show(avg_table[['Task', 'Priority', 'Category', 'Effort', 'Notes']])
