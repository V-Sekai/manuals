...
for f in files:
    ...
    # Use HTML for clickable links
    link = f'<a href="{f}">{short_title}</a>'
    rows.append({"title": link, "reading-time": reading_time, "filename": filename})

# Ensure both columns are present and sort by filename descending
df = pd.DataFrame(rows, columns=["title", "reading-time", "filename"]).sort_values("filename", ascending=False).drop(columns=["filename"])

# Show with paging (10 per page), render HTML links and show reading time
