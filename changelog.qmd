...
for f in files:
    ...
    # Make the title clickable (relative link to the markdown file)
    link = f"[{short_title}]({f})"
    rows.append({"title": link, "reading-time": reading_time, "filename": filename})

df = pd.DataFrame(rows)[["title", "reading-time", "filename"]].sort_values("filename", ascending=False).drop(columns=["filename"])
