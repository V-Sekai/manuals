...
if not effort_valid.empty:
    # Invert: lower Effort = larger marker
    effort_norm = (effort_valid.max() - df["Effort"]) / (effort_valid.max() - effort_valid.min() + 1e-6)
...
fig = px.scatter_3d(
    ...
    title="Feature Roadmap (3D): Latency vs Effort vs Priority (Pareto Frontier)",
    labels={"Latency": "Age (days)", "Effort": "Effort (1-100 Scale)", "Priority": "Priority (Higher = More Votes)", "Created_on": "Created"}
)
