...
# Use latency (age in days since Created_on) as marker size
import numpy as np
...
latency_valid = df["Latency"].dropna()
if not latency_valid.empty:
    latency_norm = (df["Latency"] - latency_valid.min()) / (latency_valid.max() - latency_valid.min() + 1e-6)
    marker_sizes = min_size + (max_size - min_size) * (latency_norm ** 3)
...
fig = px.scatter(
    ...
    title="Feature Roadmap: Priority vs Effort (Pareto Frontier, Circle Size = Latency)",
    labels={"Effort": "Effort (1-100 Scale)", "Priority": "Priority (Higher = More Votes)", "Latency": "Age (days)", "Created_on": "Created"}
)
