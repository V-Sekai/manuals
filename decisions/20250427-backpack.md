...
1. **Phase 1**: Phoenix Core & Pythonx Setup
   ...
   - Implement Oban workers that directly call librecommender functions via pythonx for initial testing and simple workflows.
2. **Phase 2**: Recommender Bridge & Membrane Integration
   ...
   - Integrate pythonx calls within Membrane pipelines, potentially triggered by Oban jobs, for more complex, stream-based processing (e.g., analyzing scene data).
3. **Validation**:
   - \<500ms p99 inference latency for recommendation calls via pythonx.
   - \>5% data serialization errors between Elixir and Python OR significant performance degradation due to pythonx overhead compared to benchmarks.
