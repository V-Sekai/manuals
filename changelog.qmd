...
for f in files:
    ...
    # Extract year from directory path for URL construction
    dir_path = os.path.dirname(f) # e.g., "changelog/2025" or "changelog"
    year_for_url_path = ""
    # Check if path is like "changelog/YYYY"
    if dir_path.startswith("changelog" + os.sep): # Handles cases like "changelog/2025"
        possible_year = os.path.basename(dir_path)
        if possible_year.isdigit() and len(possible_year) == 4:
            year_for_url_path = possible_year
            
    if year_for_url_path:
        url = f"https://v-sekai.github.io/manuals/changelog/{year_for_url_path}/{html_filename}"
    else:
        # Fallback for files directly under changelog/ (if any) or if path structure is different
